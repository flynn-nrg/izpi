...
# FreeBSD installs versioned Go binaries (e.g. go126 for go 1.26) instead of a
# plain "go" on PATH. Derive the expected binary name from go.mod and use it
# if present, falling back to "go" everywhere else.
GO_MOD_VERSION := $(shell awk '/^go [0-9]+\.[0-9]+/ {print $$2; exit}' go.mod)
GO_VERSIONED := go$(subst .,,$(GO_MOD_VERSION))
GO := $(shell command -v $(GO_VERSIONED) >/dev/null 2>&1 && echo $(GO_VERSIONED) || echo go)
...
ifneq (,$(filter FreeBSD NetBSD OpenBSD,$(UNAME_S)))
BSD_TAGS := -tags x11
endif
