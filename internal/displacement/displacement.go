...
func getDisplacementVariation(tri *minimalTriangle, displacementMap texture.Texture) float64 {
	...
	d0 := displacementMap.Value(tri.u0, tri.v0, vec3.Vec3Impl{}).Z
	d1 := displacementMap.Value(tri.u1, tri.v1, vec3.Vec3Impl{}).Z
	d2 := displacementMap.Value(tri.u2, tri.v2, vec3.Vec3Impl{}).Z
...
func applyDisplacement(in []*minimalTriangle, displacementMap texture.Texture, min, max float64) []*hitable.Triangle {
	...
	for _, tri := range in {
		...
		displacementVertex0TangentSpace := displacementMap.Value(tri.u0, tri.v0, vec3.Vec3Impl{})
		...
		displacementVertex1TangentSpace := displacementMap.Value(tri.u1, tri.v1, vec3.Vec3Impl{})
		...
		displacementVertex2TangentSpace := displacementMap.Value(tri.u2, tri.v2, vec3.Vec3Impl{})
