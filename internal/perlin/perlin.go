...
func (pl *Perlin) Noise(p *vec3.Vec3Impl) float32 {
	...
	u := p.X - math32.Floor(p.X)
	v := p.Y - math32.Floor(p.Y)
	w := p.Z - math32.Floor(p.Z)
	i := int(math32.Floor(p.X))
	j := int(math32.Floor(p.Y))
	k := int(math32.Floor(p.Z))
...
func perlinGenerate() []*vec3.Vec3Impl {
	...
	for i := range p {
		p[i] = vec3.UnitVector(&vec3.Vec3Impl{X: -1 + 2*float32(rand.Float64()), Y: -1 + 2*float32(rand.Float64()), Z: -1 + 2*float32(rand.Float64())})
	}
...
func permute(p []int) []int {
	for i := (len(p) - 1); i > 0; i-- {
		target := int(float32(rand.Float64()) * float32(i+1))
		tmp := p[i]
		p[i] = p[target]
		p[target] = tmp
	}
