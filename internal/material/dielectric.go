...
func (d *Dielectric) calculateBeerLambertAttenuation(pathLength float32, lambda float32, u, v float32, p *vec3.Vec3Impl) float32 {
	if d.spectralAbsorptionCoeff != nil {
		...
		return math32.Exp(-absorptionCoeff * pathLength)
	}
...
func (d *Dielectric) Scatter(r ray.Ray, hr *hitrecord.HitRecord, random *fastrandom.XorShift) (*ray.RayImpl, *scatterrecord.ScatterRecord, bool) {
	...
	if d.computeBeerLambertAttenuation && d.absorptionCoeff != nil && !isReflected {
		...
		attenuation = &vec3.Vec3Impl{
			X: math32.Exp(-d.absorptionCoeff.X * pathLength),
			Y: math32.Exp(-d.absorptionCoeff.Y * pathLength),
			Z: math32.Exp(-d.absorptionCoeff.Z * pathLength),
		}
	} else {
