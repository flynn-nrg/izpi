...
func getSphereUV(p vec3.Vec3Impl) (float32, float32) {
	...
	theta := math32.Asin(p.Y)
	u := 1.0 - (phi+math32.Pi)/(2.0*math32.Pi)
	v := (theta + math32.Pi/2.0) / math32.Pi
	return u, v
}
...
func (s *Sphere) HitEdge(r ray.Ray, tMin float32, tMax float32) (*hitrecord.HitRecord, bool, bool) {
	...
	ab := vec3.Dot(a, b)
	theta := math32.Acos(ab / (a.Length() * b.Length()))
	if math32.Abs(theta) <= (math32.Pi/2.0 + 0.1) {
...
func (s *Sphere) PDFValue(o vec3.Vec3Impl, v vec3.Vec3Impl) float32 {
	if _, _, ok := s.Hit((ray.New(o, v, 0)), 0.001, math32.MaxFloat32); ok {
		cosThetaMax := math32.Sqrt(1 - s.radius*s.radius/vec3.Sub(s.center0, o).SquaredLength())
